    ...
    request.get(api_endpoint + '/events?per_page=50', {
      ...
      function(err, response, body) {
        ...
        var lastFinish = body.events.findIndex(function(event){
          return event.type == "reinstall.203";
        });
        var lastStart = body.events.findIndex(function(event){
          return event.type == "reinstall.200";
        });

        res.render('status.jade', {
          status: (((lastFinish == -1 && lastStart > -1) ||
            lastFinish > lastStart) ? 'in-progress' : 'completed')});
      });
  }
